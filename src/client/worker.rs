...
use crate::{
  ...
  tdjson::ClientId,
  types::{
    AuthorizationState, CheckAuthenticationCode, CheckAuthenticationPassword, GetApplicationConfig,
    RObject, RegisterUser, SetAuthenticationPhoneNumber, Update, UpdateAuthorizationState,
  },
};
  ...
  T: TdLibClient + Send + Sync + Clone + 'static,
{
  ...
  pub async fn get_client_state(
    ...
    match &state {
      ...
      AuthorizationState::WaitOtherDeviceConfirmation(_) => Ok((ClientState::Authorizing, state)),
        ...
        match serde_json::from_value::<Update>(t) {
          ...
          Ok(update) => {
            if let Update::AuthorizationState(auth_state) = update {
              ...
              match clients.read().await.get(&client_id) {
                None => {
                  log::warn!("found updates for unavailable client ({})", client_id)
                }
                Some(ctx) => {
                  if let Some(sender) = ctx.client().updates_sender() {
                    log::trace!("sending update to client");
                    match sender.send_timeout(Box::new(update), send_timeout).await {
                      Ok(_) => {
  ...
  R: TdLibClient + Clone,
{
  ...
  let res = match state {
    ...
    AuthorizationState::WaitOtherDeviceConfirmation(wait_device_confirmation) => {
      ...
      auth_state_handler
        .handle_other_device_confirmation(client.get_auth_handler(), wait_device_confirmation)
        .await;
    ...
    AuthorizationState::WaitEmailAddress(_) => {
      todo!("wait email address is not supported")
    }
    AuthorizationState::WaitEmailCode(_) => {
      todo!("wait email code is not supported")
    }
  };
